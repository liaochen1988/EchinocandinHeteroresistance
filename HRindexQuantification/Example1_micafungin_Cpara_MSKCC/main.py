...
parser.add_argument('-d', '--detection_of_limit',
                    action='store',
                    dest='dol',
                    help='detection of limit (percentage)',
                    default=1e-4)
...
# check if detection limit is a positive number < 100
dol = float(args.dol)
if dol < 0.0 or dol > 100:
    raise RuntimeError('Detection of limit must be a float between 0 and 100. Its value is %2.2f.' % dol)
...
_ = ax01.set_ylim([-0.5, shifted_max_log10_pap + 0.5])
_ = ax01.set_yticks(np.arange(0, shifted_max_log10_pap + 1))
_ = ax01.set_yticklabels([y + np.log10(dol) for y in np.arange(0, shifted_max_log10_pap + 1)])
