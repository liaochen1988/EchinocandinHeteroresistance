...
df_info <- read.table("data/Pattern_4470.csv", sep=',', header=TRUE, row.names=1)
...
h3<- gheatmap(h2, df_info['Pattern_4470'], width=.05, offset = 0.01, colnames=F) + scale_x_ggtree() + scale_y_continuous(expand=c(0,0.3)) + scale_fill_manual(values=pal1, name='Pattern_4470')
ggsave("output/tree_hr_pattern_4470.pdf", dpi=600, width = 24, height = 24, units = "in", limitsize = FALSE)
