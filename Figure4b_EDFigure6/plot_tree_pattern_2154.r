...
df_info <- read.table("data/Pattern_2154.csv", sep=',', header=TRUE, row.names=1)
...
pal1 <- wes_palette("Darjeeling2", 5, type = "discrete")
...
h3<- gheatmap(h2, df_info['Pattern_2154_cnv_cat'], width=.05, offset = 0.01, colnames=F) + scale_x_ggtree() + scale_y_continuous(expand=c(0,0.3)) + scale_fill_manual(values=pal1, name="CPAR2_600440")
ggsave("output/tree_hr_pattern_2154.pdf", dpi=600, width = 24, height = 24, units = "in", limitsize = FALSE)
